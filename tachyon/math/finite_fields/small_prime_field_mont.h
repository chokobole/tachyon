...
class PrimeField<_Config, std::enable_if_t<!_Config::kIsSpecialPrime &&
 ...
 public:
  ...
  constexpr explicit PrimeField(uint32_t value)
      : value_(Config::Reduce(value_ * Config::kR2)) {
  ...
  constexpr bool IsOne() const { return value_ == Config::kOne[0]; }

  std::string ToString() const {
    return base::NumberToString(Config::Reduce(value_));
  }

  std::string ToHexString(bool pad_zero = false) const {
    std::string str = base::HexToString(Config::Reduce(value_));
    if (pad_zero) {
  ...
  constexpr BigInt<N> ToBigInt() const {
    return BigInt<N>(Config::Reduce(value_));
  }
  ...
  bool operator<(PrimeField other) const {
    return Config::Reduce(value_) < other.Config::Reduce(value_);
  }
  bool operator>(PrimeField other) const {
    return Config::Reduce(value_) > other.Config::Reduce(value_);
  }
