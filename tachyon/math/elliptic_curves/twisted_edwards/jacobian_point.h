#ifndef TACHYON_MATH_ELLIPTIC_CURVES_TWISTED_EDWARDS_JACOBIAN_POINT_H_
#define TACHYON_MATH_ELLIPTIC_CURVES_TWISTED_EDWARDS_JACOBIAN_POINT_H_
...
class JacobianPoint<
 ...
 public:
  ...
  constexpr JacobianPoint()
      : JacobianPoint(BaseField::Zero(), BaseField::One(), BaseField::Zero(),
                      BaseField::One()) {}
  ...
  constexpr JacobianPoint(const BaseField& x, const BaseField& y,
                          const BaseField& t, const BaseField& z)
      : x_(x), y_(y), t_(t), z_(z) {}
  constexpr JacobianPoint(BaseField&& x, BaseField&& y, BaseField&& t,
                          BaseField&& z)
      : x_(std::move(x)),
        y_(std::move(y)),
        t_(std::move(t)),
  ...
  constexpr static JacobianPoint CreateChecked(const BaseField& x,
                                               ...
                                               const BaseField& t,
                                               const BaseField& z) {
    JacobianPoint ret = {x, y, t, z};
    CHECK(ret.IsOnCurve());
    return ret;
  }

  constexpr static JacobianPoint CreateChecked(BaseField&& x, BaseField&& y,
                                               BaseField&& t, BaseField&& z) {
    JacobianPoint ret = {std::move(x), std::move(y), std::move(t),
                         std::move(z)};
    CHECK(ret.IsOnCurve());
    return ret;
  }

  // constexpr static std::optional<JacobianPoint> CreateFromX(const BaseField&
  // x,
  //                                                           bool pick_odd) {
  //   JacobianPoint point;
  ...
  constexpr const BaseField& t() const { return t_; }
  ...
  constexpr bool IsZero() const {
    return x_.IsZero() && y_ == z_ && !y_.IsZero() && t_.IsZero();
  }
...
#include "tachyon/math/elliptic_curves/twisted_edwards/jacobian_point_impl.h"

#endif  // TACHYON_MATH_ELLIPTIC_CURVES_TWISTED_EDWARDS_JACOBIAN_POINT_H_
